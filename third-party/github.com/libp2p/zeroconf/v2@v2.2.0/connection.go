...
func joinUdp6Multicast(interfaces []net.Interface) (*ipv6.PacketConn, error) {
	...
	for _, iface := range interfaces {
		if err := pkConn.JoinGroup(&iface, &net.UDPAddr{IP: mdnsGroupIPv6}); err != nil {
			fmt.Printf("Udp6 JoinGroup failed for iface %+v \n", iface)
			failedJoins++
		}
	}
...
func joinUdp4Multicast(interfaces []net.Interface) (*ipv4.PacketConn, error) {
	...
	for _, iface := range interfaces {
		if err := pkConn.JoinGroup(&iface, &net.UDPAddr{IP: mdnsGroupIPv4}); err != nil {
			fmt.Printf("Udp4 JoinGroup failed for iface %+v \n", iface)
			failedJoins++
		}
	}
	if failedJoins == len(interfaces) {
...
func listMulticastInterfaces() []net.Interface {
	...
	if err != nil {
		fmt.Printf("net.Interfaces() is nil, so get Interfaces from external, len:%d \n", len(ExternalIfaces))			// add by xiongyanfu 20241127
	...
	for _, ifi := range ifaces {
		...
		if (ifi.Flags & net.FlagMulticast) > 0 {
			...
			fmt.Printf("listMulticastInterfaces name:[%s] [%d][%d][%d][%s]\n",ifi.Name, ifi.Index,ifi.MTU, ifi.Flags, ifi.HardwareAddr.String())
		}
	}
