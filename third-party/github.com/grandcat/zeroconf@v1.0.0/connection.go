...
var (
	...
	unicastAddrIPv4 = &net.UDPAddr{
		IP:   net.IPv4zero,
		Port: 0,
	}
	keyUnicastPort = "unicast_port="
)
...
func joinUdp4Unicast(interfaces []net.Interface) (*ipv4.PacketConn, error) {
	unicastAddrIPv4.Port = 0

	udpConn, err := net.ListenUDP("udp4", unicastAddrIPv4)
	if err != nil {
		return nil, err
	}
	unicastAddrIPv4.Port = udpConn.LocalAddr().(*net.UDPAddr).Port
	pkConn := ipv4.NewPacketConn(udpConn)
	pkConn.SetControlMessage(ipv4.FlagInterface, true)
	return pkConn, nil
}
