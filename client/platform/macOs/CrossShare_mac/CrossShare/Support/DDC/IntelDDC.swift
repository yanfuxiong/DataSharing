...
public class IntelDDC {
    ...
    private static let communicationQueue = DispatchQueue(label: "com.intelddc.DDCCommunicationQueue")
    
    deinit {
        // assert(IOObjectRelease(self.framebuffer) == KERN_SUCCESS) // FIXME: it cause crash
    }
    ...
    public func write(command: UInt8, value: UInt16, errorRecoveryWaitTime: UInt32? = nil, writeSleepTime: UInt32 = 10000, numofWriteCycles: UInt8 = 2) -> Bool {
        var result = false
        IntelDDC.communicationQueue.sync {
            var success = false
    ...
    private func readInternal(command: UInt8, tries: UInt, replyTransactionType _: IOOptionBits?, minReplyDelay: UInt64?, errorRecoveryWaitTime: UInt32?, writeSleepTime: UInt32) -> (UInt16, UInt16)? {
        var result: (UInt16, UInt16)?
//        IntelDDC.communicationQueue.sync {
            var data: [UInt8] = Array(repeating: 0, count: 5)
            var replyData: [UInt8] = Array(repeating: 0, count: 11)
            var returnValue: (UInt16, UInt16)? = nil
...
//        }
    ...
    public func read(command: UInt8, tries: UInt = 1, replyTransactionType _: IOOptionBits? = nil, minReplyDelay: UInt64? = nil, errorRecoveryWaitTime: UInt32? = nil, writeSleepTime: UInt32 = 10000) -> (UInt16, UInt16)? {
        // Dummy read for flush
        let _ = readInternal(command: command, tries: tries, replyTransactionType: replyTransactionType, minReplyDelay: minReplyDelay, errorRecoveryWaitTime: errorRecoveryWaitTime, writeSleepTime: writeSleepTime)
        return readInternal(command: command, tries: tries, replyTransactionType: replyTransactionType, minReplyDelay: minReplyDelay, errorRecoveryWaitTime: errorRecoveryWaitTime, writeSleepTime: writeSleepTime)
    }
