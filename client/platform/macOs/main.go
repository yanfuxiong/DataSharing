...
func MainInit(deviceName, rootPath, downloadPath string) {
	...
	log.Printf("[%s] deviceName:[%s] rootPath:[%s] downloadPath:[%s]", rtkMisc.GetFuncInfo(), deviceName, rootPath, downloadPath)
...
func SetDisplayEventInfo(cDisplayInfoJson *C.char) {
	...
	log.Printf("[%s] PlugEvent:[%d] MacAddr:[%s] MonitorId:[%s] src:[%d] port:[%d] UdpMousePort:[%d] UdpKeyboardPort:[%d]", rtkMisc.GetFuncInfo(), displayEventInfo.PlugEvent,
...
func SetDragFileListRequest(multiFilesData string, timeStamp uint64) C.uint {
	return C.uint(rtkPlatform.GoDragFileListRequest(multiFilesData, timeStamp))
}
