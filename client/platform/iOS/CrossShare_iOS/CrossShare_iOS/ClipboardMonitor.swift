...
class ClipboardMonitor {
    ...
    private func checkClipboard() {
        ...
        if pasteboard.changeCount != lastChangeCount {
            ...
            if ClipboardMonitor.skipChecking {
                print("[Clipboard] skip local copy event")
                return
            }
    ...
    private func handleClipboardChange() {
        let pasteboard = UIPasteboard.general
        print("[Clipboard] Local copy event type: \(pasteboard.types)")
        if pasteboard.hasImages {
            if let copiedImage = pasteboard.image,let imageBase64 = copiedImage.imageToBase64() {
                print("[Clipboard] Local copy image event")
                SendImage(imageBase64.toGoString())
            } else {
                print("[Clipboard][Err] Unknown image type")
            }
        }
        else if pasteboard.hasStrings {
            if let copiedText = pasteboard.string {
                print("[Clipboard] Local copy text event: \(copiedText)")
                SendText(copiedText.toGoString())
            } else {
                print("[Clipboard][Err] Unknown text type")
            }
        }
    }
