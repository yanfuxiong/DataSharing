...
class P2PManager {
    ...
    func initConn() {
        ...
        guard let diasId = UserDefaults.get(forKey: .DEVICECONFIG_DIAS_ID),
                let src = UserDefaults.getInt(forKey: .DEVICECONFIG_SRC),
                let port = UserDefaults.getInt(forKey: .DEVICECONFIG_PORT) else {
            return
        }
        setupDeviceConfig(diasId, src, port)
    }
    ...
    private func goSetCallBackMethod() {
                        ...
                        do {
                            if let imageData = Data(base64Encoded: copiedText, options: .ignoreUnknownCharacters) {
                                if let image = UIImage(data: imageData) {
                                    ...
                                    print("Failed to decode Base64 string into image")
                                }
                            } else {
                                print("Failed to decode Base64 string")
                            }
                        } catch {
                            print("❌ Failed to base64ToImage: \(error.localizedDescription)")
                        }
                    }
                }
            }
        ...
        autoreleasepool {
            SetCallbackMethodStartBrowseMdns { (cinstance: UnsafeMutablePointer<CChar>?, cserviceType: UnsafeMutablePointer<CChar>?) in
                print("[GO][Callback] SetCallbackMethodStartBrowseMdns")
                ...
                P2PManager.shared.initConn()
            }
        }
        autoreleasepool {
            SetCallbackMethodStopBrowseMdns {
                print("[GO][Callback] SetCallbackMethodStopBrowseMdns")
                DispatchQueue.main.async {
        ...
        autoreleasepool {
            SetCallbackMethodFoundPeer {
                print("[GO][Callback] SetCallbackMethodFoundPeer")
        ...
        autoreleasepool {
            SetCallbackMethodFileConfirm { (cid: UnsafeMutablePointer<CChar>?, cplatform: UnsafeMutablePointer<CChar>?, cfileName: UnsafeMutablePointer<CChar>?, cfileSize: Int64) in
                print("[GO][Callback] SetCallbackMethodFileConfirm")
                if let cid = cid, let cplatform = cplatform, let cfileName = cfileName {
                    ...
                    print("[P2PManager][FileDrop] ID:[\(id)], Platform:[\(platform)], FileNmae:[\(fileName)], FileSize:[\(cfileSize)]")
                    let isReceive = 1
        ...
        autoreleasepool {
            SetEventCallback { event in
                print("[GO][Callback] SetLogMessageCallback \(event)")
            }
        }
...
//                    print("[GO][Callback] SetCallbackUpdateProgressBar id:\(fileIdString) fileName:\(fileNameString) recvSize:\(recvSize) bytes total:\(total) bytes")
                    var downloadParams = [:]
...
//                    print("[GO][Callback] SetCallbackUpdateProgressBar ip:\(ipString) id:\(idString) deviceName:\(deviceNameString) currentfileName:\(currentfileNameString) recvFileCnt:\(recvFileCnt) totalFileCnt:\(totalFileCnt) currentFileSize:\(currentFileSize) totalSize:\(totalSize) recvSize:\(recvSize) timestamp:\(timestamp)")
                    var downloadParams = [:]
                    let downloadItem = DownloadItem()
                    downloadItem.timestamp = TimeInterval(timestamp)
                    downloadItem.uuid = "\(timestamp)"
    ...
    func startP2PService(config: P2PConfig = .defaultConfig()) {
        guard !isRunning else {
            print("P2P service is already running")
            return
        }
        ...
        p2pQueue?.async { [weak self] in
            ...
            print("Server IP: \(config.serverIpInfo)")
            print("Listen Host: \(config.listenHost)")
            print("Starting P2P service on port \(config.listenPort)...")
            do {
    ...
    func stopP2PService() {
        guard isRunning else {
            print("P2P service is not running")
            return
        }
        p2pQueue = nil
        isRunning = false
    ...
    private func handleClientList(_ clientListStr: String) {
        ...
        for (index, clientStr) in clientListArr.enumerated() {
            ...
            if clientArr.count != 3 {
                print("[P2PManager][Err] Invalid client count = \(clientArr.count)")
                continue
            }
            ...
            if ip.isEmpty || id.isEmpty || name.isEmpty {
                continue
            }

            let clientInfo = ClientInfo(ip: ip, id: id, name: name)
            mClientList.append(clientInfo)
            print("[P2PManager] update client list[\(index)]: IP:[\(ip)], Name:[\(name)], ID:[\(id)]")
        }
    ...
    func setupDeviceConfig(_ diasId: String,_ src: Int,_ port: Int) {
        DispatchQueue.main.async {
            print("[P2PManager] setup device config. DIAS_ID:[\(diasId)], (Source, Port):[\(src), \(port)]")
    ...
    func setFileDropRequest(filePath: String, id: String, fileSize: Int64) {
        if filePath.isEmpty || id.isEmpty || fileSize <= 0{
            print("[P2PManager] file drop request failed: Invalid params")
        }
        DispatchQueue.main.async {
