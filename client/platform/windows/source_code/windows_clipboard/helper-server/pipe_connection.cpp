...
void PipeConnection::processAnyMsg(const AnyMsg &message)
{
    const int funcCode = message.funcCode;
    switch (funcCode) {
    case AnyMsgFuncCode::UpdateDownloadPath_code: {
        UpdateDownloadPathMsg newMessage;
        if (UpdateDownloadPathMsg::fromByteArray(message.msgData, newMessage)) {
            QVariant sendData = QVariant::fromValue<UpdateDownloadPathMsgPtr>(std::make_shared<UpdateDownloadPathMsg>(std::move(newMessage)));
            Q_ASSERT(sendData.canConvert<UpdateDownloadPathMsgPtr>() == true);
            Q_EMIT CommonSignals::getInstance()->dispatchMessage(sendData);
        }
        break;
    }
    case AnyMsgFuncCode::UpdateLocalConfigInfo_code: {
        UpdateLocalConfigInfoMsg newMessage;
        if (UpdateLocalConfigInfoMsg::fromByteArray(message.msgData, newMessage)) {
            QVariant sendData = QVariant::fromValue<UpdateLocalConfigInfoMsgPtr>(std::make_shared<UpdateLocalConfigInfoMsg>(std::move(newMessage)));
            Q_ASSERT(sendData.canConvert<UpdateLocalConfigInfoMsgPtr>() == true);
            Q_EMIT CommonSignals::getInstance()->dispatchMessage(sendData);
        }
        break;
    }
