...
void ProcessMessage::onRecvServerData(const QByteArray &data)
{
    ...
    while (g_getCodeFromByteArray(QByteArray(m_buffer.peek(), m_buffer.readableBytes()), typeValue, code)) {
        ...
        case AnyMsg_Code: {
            AnyMsg message;
            if (AnyMsg::fromByteArray(QByteArray(m_buffer.peek(), m_buffer.readableBytes()), message)) {
                m_buffer.retrieve(message.getMessageLength());
                processAnyMsg(message);
            }
            break;
        }
        default: {
...
void ProcessMessage::processAnyMsg(const AnyMsg &message)
{
    const int funcCode = message.funcCode;
    switch (funcCode) {
    case AnyMsgFuncCode::UpdateClientVersion_code: {
        UpdateClientVersionMsg newMessage;
        if (UpdateClientVersionMsg::fromByteArray(message.msgData, newMessage)) {
            QVariant sendData = QVariant::fromValue<UpdateClientVersionMsgPtr>(std::make_shared<UpdateClientVersionMsg>(newMessage));
            Q_ASSERT(sendData.canConvert<UpdateClientVersionMsgPtr>() == true);
            Q_EMIT CommonSignals::getInstance()->dispatchMessage(sendData);
        }
        break;
    }
    case AnyMsgFuncCode::ShowWindowsClipboard_code: {
        ShowWindowsClipboardMsg newMessage;
        if (ShowWindowsClipboardMsg::fromByteArray(message.msgData, newMessage)) {
            QVariant sendData = QVariant::fromValue<ShowWindowsClipboardMsgPtr>(std::make_shared<ShowWindowsClipboardMsg>(std::move(newMessage)));
            Q_ASSERT(sendData.canConvert<ShowWindowsClipboardMsgPtr>() == true);
            Q_EMIT CommonSignals::getInstance()->dispatchMessage(sendData);
        }
        break;
    }
    case AnyMsgFuncCode::NotifyErrorEvent_code: {
        NotifyErrorEventMsg newMessage;
