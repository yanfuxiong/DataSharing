...
void DirectoryJumpWidget::updateUI()
{
    ...
    for (const auto &itemName : dirList) {
        ...
        itemButton->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        itemButton->setFixedWidth(QFontMetrics(font()).horizontalAdvance(itemName + QByteArray(4, ' ')));
...
void FileTableView::processMouseMovingTask()
{
    ...
    if (pathList.isEmpty() == false) {
        if (MonitorPlugEvent::getInstance()->getCacheMonitorData().macAddress.empty()) {
            Q_EMIT CommonSignals::getInstance()->showWarningMessageBox("warning", "It's not a DIAS monitor!!!");
            return;
        }
...
void FileExplorer::onDragFiles(const QStringList &pathList, const QPoint &pos)
{
...

    {
        ...
        auto func = [&] {
            return MonitorPlugEvent::updateMousePos(MonitorPlugEvent::getInstance()->getCacheMonitorData().hPhysicalMonitor,
